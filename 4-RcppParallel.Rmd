...
Aqui vou usar o exemplo da [documentação do `parallelFor` no site do `{RcppParallel}`](http://rcppcore.github.io/RcppParallel/index.html#parallelfor), mas vou adicionar alguns comentários para você entender o que está sendo feito.
...
A lógica do `parallelReduce` é similar ao `parallelFor`. Primeiro ambos usam um `Worker`
...
struct sum_of_squares : public Worker
{
  ...
  // Construtor do Objeto Worker
  sum_of_squares(const NumericVector input, double output) 
    : input(input), output(output) {}
  
  // Overload do operador ()
  void operator()(std::size_t cbegin, std::size_t cend) {
      std::transform(input.begin() + begin, 
                     input.begin() + end, 
                     output.begin() + begin, 
                     ::sqrt);
